...
Client.prototype.onChildrenChange = function(data, callback) {
  ...
  async.each(Object.keys(data), function(path, next) {
    self.setState(path, data[path], next);
  }, callback);
};
...
Client.prototype.getZKData = function(path, callback) {
  var self = this;
  this.zkClient.getData(this.zkPath + '/' + path, function(err, data) {
    if (err) {
...
Client.prototype.watchZkData = function(path) {
  var self = this;
  var fullPath = self.zkPath + '/' + path;
  self.zkClient.getData(fullPath, function(event) {
    if (event.type == Event.NODE_DATA_CHANGED) {
      self.getZKData(fullPath, function(data) {
