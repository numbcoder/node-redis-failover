...
WatcherManager.prototype.promoteRedisMaster = function(nodes, index, callback) {
  ...
  if (index >= nodes.length) {
    logger.error('no redeis node can be promote to be Master!');
    return callback();
  }
...
WatcherManager.prototype.updateRedisState = function(data) {
  ...
  self.updateNodesInfo(function() {
    ...
    if (!self.checkState(redisState)) {
      ...
      if (self.masterNode.password) {
        self.redisState['password'] = self.masterNode.password;
      }
      self.zk.setData(self.rootPath, self.redisState, function(err) {
        if (err) {
