...
function WatcherManager(opts) {
  ...
  opts.RedisPassword || (this.redisPassword = opts.RedisPassword);
...
WatcherManager.prototype._ready = function() {
  ...
  this.getLock(function() {
    ...
    self.redisServers.forEach(function(i) {
      i = i.split(':');
      self.addNode({host: i[0], port: i[1], password: self.redisPassword});
    });
...
WatcherManager.prototype.updateRedisState = function(data) {
  ...
  self.masterNode.updateInfo(function() {
    var slaves = [];
    data.available.forEach(function(name) {
      if (name == self.masterNode.name) return;

      if (self.nodes[name].available && self.masterNode.slaves.indexOf(name) > -1) {
        slaves.push(name);
      } else {
